/*문제1. 직원들의 사번(employee_id), 이름(firt_name), 성(last_name)과 
...
from    employees em left outer join departments de
        ...
        left outer join jobs js
        on em.job_id = js.job_id
order by    em.employee_id asc;

/*문제3. 도시별로 위치한 부서들을 파악하려고 합니다.
도시아이디, 도시명, 부서명, 부서아이디를 도시아이디(오름차순)로 정렬하여 출력하세요 
부서가 없는 도시는 표시하지 않습니다. (27건)*/
select  lo.location_id "도시 아이디",
        lo.city "도시 이름",
        de.department_name "부서 이름",
        de.department_id "부서 아이디"
...
select  lo.location_id "도시 아이디",
        ...
        de.department_id "부서 아이디"
from    countries co,
        locations lo left outer join departments de
        on lo.location_id = de.location_id
where   co.country_id = lo.country_id
order by    lo.location_id asc;

/*문제4. 지역(regions)에 속한 나라들을 지역이름(region_name), 
나라이름(country_name)으로 출력하되 지역이름(오름차순), 
나라이름(내림차순) 으로 정렬하세요. (25건)*/
select  re.region_name "지역 이름",
...
나라명(오름차순)로 정렬하여 출력하세요. 값이 없는 경우 표시하지 않습니다.
(27건)*/
select  co.country_name "나라 이름",
        co.country_id "나라 아이디",
        lo.city "도시 이름",
...
/*문제8. 각 부서(department)에 대해서 부서번호(department_id), 
부서이름(department_name), 매니저(manager)의 이름(first_name), 
